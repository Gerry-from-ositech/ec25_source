...
function OnChangePubSafety( select)  {
    if (select.selectedIndex != -1 && select.selectedIndex != orgPubSafteyIndex) {
         dirty = true;
         document.getElementById('Save').disabled=false;
    }
}
...
function ValidatePage(url) {
 ...
 if (edit_mode == false) {
  ...
  if (cell_type == 'PLS62_W'){
      document.getElementById('DiffServ').value = 'None (00)';
      document.getElementById('bandwidth_limit').value = 'None';  // hardcoded value
    // check if Public Safety is activate for PLS8 or PXS8, if not validate DiffServ entry
  } else if (document.getElementById('PublicSafety').style.display == 'none'){
    var re = /^[0-9a-fA-F]+$/;
    var val;
...
// If any field was invalid, do not submit this form
  ...
  //if (cell_type != "ELS31_V" && gobi_index != -1) {
   if( edit_mode == false){
...
/* HTMLEncode  by using browseres own automatic Html converter
* required for IE8
*/
...
function ReactivateCdma() {
   ...
   //   }
      ...
      // Response:     // OK - UTMS flip was succesful
      // Active - CDMA flip was succesfull and Radio is activated
...
function SetupDiffServ(){
        document.getElementById("selvalue").disabled = false;
        document.getElementById("DIFFSERV").style.display = "block";
        document.getElementById("selvalue").value = safety_type;

        var elps = document.getElementById('select_opts');
...
<div id='ajaxwin' style='display:none;margin: 0 20px 0 13em'>
     ...
     <div id='progress' style='display:none;'><span id='runningMsg'>Please Wait &nbsp;&nbsp;&nbsp;</span><br /> <img id='waitbar' src='/images/barwait.gif'></div>
...
<div id='mainContent'>
   ...
   <div align='left'class='text'><label for='backhaul'>Backhaul: </label>
   <input onClick= 'enableSave();' type='checkbox' name='check_backhaul' id='check_backhaul' checked='checked' >
...
var presets = CellProviderPresets.length;
if (presets > 12) presets = 12;
list.size = presets.toString();
if ( presets == 1){
