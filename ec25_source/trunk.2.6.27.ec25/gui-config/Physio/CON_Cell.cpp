...
//TODO: add the APN strings for LTE PLS8 for Verizon and Sprint
  ...
  if (s== "NO" || s == "PXS8"){
     if ( FileExists("/usr/sbin/http/cgi-bin/enable_sprint") )
        ...
        printf("var CellProviderPresets = [  ['Verizon','','','',8],   ['Verizon (KORE)','','','',8] ,  ['AT&T','','','isp.cingular',1], ['AT&T (KORE)','','','c2.korem2m.com',1],    ['Bell','','','inet.bell.ca',0], ['Other (GSM/UMTS)','','','REQUIRES APN STRING',0] ];\n");
  } else if(s == "ELS31_V"){
      ...
      if ( FileExists("/usr/sbin/http/cgi-bin/enable_sprint") )
        ...
        printf("var CellProviderPresets = [  ['Verizon','','','vzwinternet',8], ['Verizon (KORE)','','','wyleslte.gw7.vzwentp',8], ['Verizon (Public Safety)','','','REQUIRES APN STRING',8], ['AT&T','','','broadband',1], ['AT&T (KORE)','','','10569.mcs',1],  ['Bell','','','inet.bell.ca',0], ['Rogers','','','ltemobile.apn',0], ['Telus','','','isp.telus.com',0], ['Other (LTE)','','','REQUIRES APN STRING',0] ];\n");  
  ...
  sTemp  = iniFile.ReadString(con_name,"largeupload","NO");
  printf("var largeupload = '%s';\n", sTemp.c_str());

  sTemp  = iniFile.ReadString(con_name,"backhaul","NO");
...
//   printf("var cell_type = '%s';\n", s.c_str());
  ...
  //    printf("var cell_type = 'PXS8';\n");
   ...
   fp = popen( "cat /usr/sbin/http/cgi/CON_Cell.cgi", "r");
   if (fp != NULL) {
